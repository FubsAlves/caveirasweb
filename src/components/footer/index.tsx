        ...
        <>
            <div className="w-full fixed z-[1001] bottom-0"> 
                ...
                <div className={`flex items-center justify-center w-full msm:h-10 mdm:h-12 mlg:h-16 shadow-2xl shadow-inner shadow-stone-900" ${!pathname.includes("/menu/") ? "bg-caveirito" : "bg-[#F5F5F5]"}`}>
                        { pathname.includes('/menu/') ? <><Button className={classes.button} disabled={currentSnack?.isActive ? false : true} color={`${pathname.includes("menu/Chickens") ? "#f07100" : "#B71105"}`} style={{ color: '#FFF', fontWeight: "bolder" }} variant="filled" radius="md" onClick={() => {
                        addItem({
                            id: currentSnack.id,
