...
import { useBagDelayStore } from '@/store/BagStatusStore';
...
export default function Bag ({opened} : BagProps) {
    ...
    return (
        <>
            ...
            >
            {(styles) => 
            <div style={styles} className="flex flex-row items-center justify-around w-full msm:h-8 mdm:h-10 mlg:h-16 bg-[#870018] hover:cursor-pointer text-white" onMouseEnter={() => {clearDelay()}} onMouseLeave={() => {setDelay()}} onClick={() => {open(); clearDelay()}}>
...
// @ts-ignore */}
                    ...
                    <div className='flex flex-col text-caveirito font-semibold'>
                        <h3>Total:</h3>
                        <h4><Total/> + Consultar as taxas de entrega</h4>
